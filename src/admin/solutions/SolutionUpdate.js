...
export default function SolutionUpdate() {
  ...
  useEffect(() => {
    fetchDataAdmin(controllers.solutions + id, setEditing, () => {
      setTimeoutCkeditor200();
      setupCkfinder();
    });
  }, []);
  if (!editing.Link)
    return (
      <div>
        <h2 className="text-danger">This solution does not exist</h2>
    ...
    <div className="containter mx-2">
      <div>
        ...
        <button
          ...
          onClick={(e) => put(controllers.solutions, editing, backLink)}
          data-dismiss="modal"
        >
          Update
        </button>
